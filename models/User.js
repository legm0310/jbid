...
userSchema.getAll = function (filter, projection, callback) {
  var user = this;
  user.find({ "userKey": filter  })
}

userSchema.getUserByEmail = function (email, callback) {
  const qurey = {
    email: email
  };
  userSchema.findOne(qurey, callback);
};
