...
async def create_did_and_write_nym():
    try:
        ...
        # await pool.create_pool_ledger_config(config_name=pool_name, config=None)
        # 1.
        ...
        steward_seed = 'trustee3trustee3trustee3trustee3'
        ...
        did_seed = "00000000000000000STUDENT" + student_seed
        ...
        get_nym = await ledger.build_get_nym_request(submitter_did=trust_anchor_did,target_did=trust_anchor_did)
        # get_nym_response = await ledger.parse_get_nym_response(response=get_nym)

        get_nym_response = await ledger.submit_request(pool_handle=pool_handle, request_json=get_nym)
        pprint.pprint(json.loads(get_nym_response))
        ...
        print(json.dumps(user_data, ensure_ascii=False, indent="\t"),end='')
